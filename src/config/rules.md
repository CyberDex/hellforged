...
- The balance can also be set by hand, on the entry the menu lets down inside itself (`src/ui/Balance.tsx`), which the balance on the bar opens straight onto — and which opens of its own accord as the balance runs out. What is typed there is whole and never below zero, and the game settles around it the way it settles after a spin: the top of the slider comes down to it, and a balance that had run out is playable again. A figure set while the reels are still turning goes onto the balance at once but only settles the machine as that spin ends, since its stake was taken and its win worked out before the change.
- The bet is set on the slider under the `Bet` pannel, which runs from `settings.minBet` to `settings.maxBet` (currently 1 to 1 000 000) a unit at a time and opens at `settings.defaultBet`. The pannel reads out whatever it is dragged to.
...
- A spin stays pending between taking its stake and crediting its win. Once its answer arrives, the symbols and win are kept with that pending spin until the reels finish. A reload credits an answer already received, or refunds the stake when the request was cut off before its answer; either settlement clears the pending spin with the balance change, so another reload cannot apply it twice.
...
- The `Win` pannel is only written once the announcement over the reels has counted all the way up to the win, so the figure is revealed over the reels first and read out beside them after; it then keeps it until the next spin starts. A reveal that never got to finish counting — a tab left in the background — still leaves the win on the pannel as the game returns to idle.
...
- A win is held for `settings.winDuration` and counts up over `settings.winCountDuration` of it, so the amount settles well before the announcement comes down.
- A win that reaches `settings.bigWinStages` is a big one: it falls into the last stage its amount reaches and is held for that stage's `reveals` times the usual hold (currently 2, 3 and 4, so 5000, 7500 and 10000ms), counting up over all of it but the last `settings.bigWinHold` (currently 1000ms), so it ends standing on the figure it arrived at. It is read out from inside the anticipation zoom, which is only dropped once the reveal is over.
- Each stage is announced under its own title — `BIG WIN` from 1000, `MEGA WIN` from 10000, `EPIC WIN` from 100000 — and it is the climbing figure that changes the announcement over rather than the win it is climbing to, so it is rewritten mid-count, on the frame the number passes a stage's figure. The amount, not the symbols, decides it: a pair staked high enough is a big win and a filled line at the minimum is not.
- From the first of those figures on, coins shower out from behind the announcement (`src/layout/components/CoinShower.ts`): one is thrown off the number for every figure the count climbs through — the same beat the coin is heard on — up out of wherever the amount currently reads, spinning about its own upright, and pulled back down under its own weight. The shower ends with the count, the last coins falling out of it, and anything still in the air when the announcement is written over or comes down goes with it.
...
- The bar drops a menu out of itself (`src/ui/Menu.tsx`), cut to the bar rather than laid over the machine: what is set about the game rather than played of it. Everything on it is the player's own and is kept in the browser (`localStorage`, under `<game name>.sound` and `<game name>.graphics`), the way the balance and the bet are.
