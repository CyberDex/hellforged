...
- `Game rules` opens the rules the player is told (`src/ui/Rules.tsx`), which are their own document — `src/config/rules.player.md`, rendered to the sheet as the game loads. This file is the same game written out for whoever works on it, down to the setting and the source file each rule lives in, which is not what somebody who only wants to know what a symbol pays came to read.
