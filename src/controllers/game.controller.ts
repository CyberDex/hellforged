...
class GameController {
    ...
    init(layout: RootLayout) {
        ...
        gameStore.subscribe((current, previous) => {
            ...
            // The announcement lasts as long as the reveal does.
            if (state === 'idle') winLayout.hide();
        });
    }
    ...
    private spin() {
        ...
        // in play, which is what the last reel is drawn out for.
    ...
    private reelStopped() {
        ...
        if (this.#anticipating) {
            ...
            // from here, over the gap between the two stops, and jumps back
            // the moment the held-back reel lands.
            ...
            } else if (this.#landed === settings.reels) {
                this.#layout?.unzoom();
            }
        }
        ...
        setTimeout(() => this.setState('idle'), settings.winDuration);
    }
